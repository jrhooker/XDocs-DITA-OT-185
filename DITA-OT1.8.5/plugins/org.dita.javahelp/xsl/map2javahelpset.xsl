...
<xsl:param name="WATERMARKING" select="0"/>    
...
<xsl:output
  ...
  <xsl:template match="*[contains(@class, ' map/map ')]">
    ...
    <helpset version="2.0">
      ...
      <xsl:if test="number($WATERMARKING)= 1"> <xsl:text> </xsl:text>UNENCRYPTED_REPLACEMENT_WATERMARKING_STRING<xsl:text> </xsl:text> </xsl:if>
      </title>
      <maps>
        <homeID>
