...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*" mode="retrieveReferenceTitle" >
        <xsl:choose>
            <xsl:when test="*[contains(@class,' topic/title ')]"><xsl:value-of select="normalize-space(string(*[contains(@class, ' topic/title ')]))"/></xsl:when>
            <xsl:otherwise>
        ...
        <xsl:call-template name="insertVariable">
            ...
            <xsl:with-param name="theParameters">
                <xsl:variable name="tablecount">
                    <xsl:value-of
                        select="count(preceding::*[contains(@class, ' topic/table ')][child::*[contains(@class, ' topic/title ')]])"
                    />
                </xsl:variable>
                <xsl:variable name="revhistorycount">
                    <xsl:value-of
                        select="count(preceding::*[contains(@class, ' topic/table ')][descendant::pmc-revhistory])"
                    />
                </xsl:variable>
                <number>
    ...
    <xsl:template name="preceding-count-li-reference">
        ...
        <xsl:choose>
            <xsl:when
                test="
                    $element/parent::*[contains(@class, ' topic/ol ')]/
                ...
                <xsl:variable name="temp-ol"
                    select="
                        count($element/parent::*[contains(@class, ' topic/ol ')]/
                ...
                <xsl:call-template name="preceding-count-li-reference">
                    ...
                    <xsl:with-param name="current-li-count"
                        select="number($current-li-count) + number($temp-ol)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:when
                test="
                ...
                <xsl:variable name="temp-ol"
                    select="
                        count($element/parent::*[contains(@class, ' topic/ol ')]/preceding-sibling::*[contains(@class, ' topic/ol ')]
            ...
            <xsl:choose>
                ...
                <xsl:when
                    ...
                    <xsl:variable name="tnumber">
                        <xsl:value-of
                            select="count(//*[@id = $element/@id]/preceding-sibling::*) + 1"/>
                    </xsl:variable>
                    ...
                    <xsl:value-of select="number($tnumber) + number($preceding-count)"/>

                    <xsl:text> ] </xsl:text>
                </xsl:when>
                <xsl:otherwise><xsl:apply-templates select="$element" mode="retrieveReferenceTitle"/></xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template name="insertPageNumberCitation">
        ...
        <xsl:choose>
            <xsl:when test="not($element) or ($destination = '')"/>
            <xsl:when
                test="contains($element/@class, ' topic/li ') and contains($element/parent::*/@class, ' topic/ol ')"/>
    ...
    <xsl:template match="*[contains(@class, ' topic/xref ')]" name="topic.xref">
        ...
        <fo:basic-link xsl:use-attribute-sets="xref">
            ...
            <xsl:choose>
                <xsl:when
                    test="not(@scope = 'external' or not(empty(@format) or @format = 'dita')) and not($referenceTitle = '')">
        ...
        <xsl:if
            test="not(@scope = 'external' or not(empty(@format) or @format = 'dita')) and not($referenceTitle = '') and not($element[contains(@class, ' topic/fn ')])">
