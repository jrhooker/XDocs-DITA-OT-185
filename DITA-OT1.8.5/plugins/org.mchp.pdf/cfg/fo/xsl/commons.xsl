...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:if test="$level eq 1">
                        <fo:marker marker-class-name="current-header"><xsl:for-each
                                select="child::*[contains(@class, ' topic/title ')]">
                                <xsl:apply-templates select="." mode="getTitleNoNumHeader"/>
                            </xsl:for-each>
                        </fo:marker>
                    </xsl:if>
    ...
    <xsl:template match="*" mode="getTitleNoNumHeader">
        <xsl:variable name="topic"
            select="ancestor-or-self::*[contains(@class, ' topic/topic ')][1]"/>
        <xsl:variable name="id" select="$topic/@id"/>
        <xsl:variable name="mapTopics" select="key('map-id', $id)"/>
