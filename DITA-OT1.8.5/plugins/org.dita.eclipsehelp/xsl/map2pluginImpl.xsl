...
<xsl:import href="custom/custom.xsl" />    
...
<xsl:text>&#10;</xsl:text></xsl:variable>
  ...
  <xsl:template match="*[contains(@class, ' map/map ')]" mode="eclipse.plugin">
    <xsl:variable name="concatfilters"><xsl:call-template name="concatfilters"/></xsl:variable>
    <xsl:element name="plugin">
     <xsl:attribute name="id">
        <xsl:choose>
          <xsl:when test="@id">
            <xsl:value-of select="concat(@id, $concatfilters)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>org.sample.help.doc</xsl:text>
            <xsl:call-template name="output-message">
    ...
    <xsl:choose>
      <xsl:when test="$plugin='true'">
        ...
        <xsl:choose>
          ...
          <xsl:when test="@id">
            <xsl:text>Bundle-SymbolicName: </xsl:text><xsl:value-of select="concat(@id, $concatfilters)"/>;<xsl:text> singleton:=true</xsl:text><xsl:value-of select="$newline"/>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="@id">                        
            ...
            <xsl:if test="$fragment.lang=''">
              <xsl:text>Bundle-SymbolicName: </xsl:text><xsl:value-of select="concat(@id, $concatfilters)"/><xsl:value-of select="$newline"/>
            </xsl:if>
          </xsl:when>
          <xsl:otherwise>
