...
<xsl:param name="SHOWBORDERS">0</xsl:param>
     ...
     <xsl:if test="$SHOWBORDERS = 1">
      ...
      <fo:block text-align="end" margin-left="0cm" margin-right=".5cm" margin-bottom="3.5cm"
         space-before="0mm" space-after="0mm">
          <fo:external-graphic
            src="url({concat($customizationDir.url, 'Branding/Microchip/Horizontal_logo_w_tagline.png')})"
            height="2.75cm" content-width="scale-to-fit" content-height="scale-to-fit"/>
      </fo:block>
   </xsl:template>
   ...
   <xsl:template name="genericFooter">     
      ...
      <fo:block text-align="end" margin-left="-.5cm" space-before="0mm" space-after="0mm"
         margin-right="0mm">
         <fo:table width="17.5cm" display-align="before" border-top-color="{$title.border.font.color}" border-top-style="solid" border-top-width="1pt">
            <xsl:if test="$SHOWBORDERS = 1">
            ...
            <fo:table-body>
               <fo:table-row>
                  ...
                  <fo:table-cell text-align="center" padding-before="3mm" padding-after="8mm">
                     ...
                     <fo:block font-family="{$header.font.family}" font-size="{$footer.font.size}" color="{$header.font.color}" hyphenate="false">
                        <xsl:choose>
                           <xsl:when test="//pmc_iso[1]/pmc_footertext">
                              <xsl:value-of select="//pmc_iso[1]/pmc_footertext"/><xsl:text> </xsl:text>
                           </xsl:when>
                           <xsl:otherwise>
                        ...
                        <xsl:if test="//pmc_iso[1]/pmc_productnumber">
                           <xsl:text>  </xsl:text>
                           <xsl:value-of select="//pmc_iso[1]/pmc_productnumber"/><xsl:text> </xsl:text>
                        </xsl:if>
                        <xsl:if test="//pmc_iso[1]/agile_part_number">
                           <xsl:text>  </xsl:text>
                           <xsl:value-of select="//pmc_iso[1]/agile_part_number"/><xsl:text> </xsl:text>
                        </xsl:if>                        
                        <xsl:if test="//pmc_iso[1]/pmc_doc_type">
                           <xsl:text>  </xsl:text>
                           <xsl:value-of select="//pmc_iso[1]/pmc_doc_type"/><xsl:text> </xsl:text>
                        </xsl:if>                        
   ...
   <xsl:template name="insertLegalEvenFooter">
      ...
      <fo:static-content flow-name="even-legal-footer">
         <fo:block xsl:use-attribute-sets="__body__even__footer">
            <xsl:call-template name="legalFooter">
               <xsl:with-param name="caller">insertLegalEvenFooter&gt;insertLegalEvenFooter</xsl:with-param>
            </xsl:call-template> 
         </fo:block>
      </fo:static-content>
   </xsl:template>
