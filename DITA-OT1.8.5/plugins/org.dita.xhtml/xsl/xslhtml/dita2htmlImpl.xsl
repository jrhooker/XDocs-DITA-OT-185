...
<!-- Parameter for watermarking javahelp files -->
  <xsl:param name="WATERMARKING" select="1"/>
  <xsl:param name="ARGS.INPUT"/><!-- the input ditamap --> 
  <xsl:param name="DITATEMPDIR"></xsl:param>
...
<xsl:param name="TRANSTYPE" select="'xhtml'"/>
  ...
  <xsl:param name="dita-css">
    <xsl:choose>
      <xsl:when test="$TRANSTYPE = 'javahelp'">commonltr-javahelp.css</xsl:when>
      <xsl:otherwise>commonltr.css</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  
  <xsl:param name="bidi-dita-css"><!-- bidirectional languages -->
    <xsl:choose>
      <xsl:when test="$TRANSTYPE = 'javahelp'">commonrtl-javahelp.css</xsl:when>
      <xsl:otherwise>commonrtl.css</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  ...
  <xsl:variable name="pre-id">
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:value-of select="generate-id()"/></xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  <!-- This div tag is what provides the button for the copy content command. -->
  <div class="dt-buttons">
    <a class="dt-button" href="javascript:;" onclick="selectElementContents(document.getElementById('{$pre-id}'));">
      <span>Copy</span></a>        
  </div>
  <pre>
    ...
    <!-- If there is an ID value present, it will be reset by the setidname in the template called after this one. All this does is make sure that there is indeed SOME id value. -->
...
<xsl:template name="topic-image">
  ...
  <xsl:choose>
      <xsl:when test="$isSVG">
        ...
        <xsl:choose>
          <xsl:when test="$TRANSTYPE = 'javahelp'">
            <img>
              <xsl:call-template name="commonattributes">
                <xsl:with-param name="default-output-class">
                  <xsl:if test="@placement = 'break'"><!--Align only works for break-->
                    <xsl:choose>
                      ...
                      <xsl:otherwise>imageleft</xsl:otherwise>
                    </xsl:choose>
                  </xsl:if>
                </xsl:with-param>
              </xsl:call-template>
              ...
              <xsl:variable name="svg-path"><xsl:call-template name="rename-svg"><xsl:with-param name="href" select="@href"/></xsl:call-template></xsl:variable>
              <xsl:attribute name="src"><xsl:value-of select="$svg-path"/>.png</xsl:attribute>
          ...
          <xsl:when test="$TRANSTYPE = 'xhtml' or $TRANSTYPE = 'eclipsehelp' or $TRANSTYPE = 'webhelp'">            
            ...
            <!-- The templates for extracting the native height and width of the svg do work, but it appears
            that the best solution is simply to span the section that contains them half way across the page
            and then span the svg itself all the way across that.-->
            <!--<xsl:variable name="svg-content" select="document(@href)"></xsl:variable>
            <xsl:variable name="svg-height"><xsl:apply-templates select="$svg-content" mode="height"/></xsl:variable>
            <xsl:variable name="svg-width"><xsl:apply-templates select="$svg-content" mode="width"/></xsl:variable> -->                
            <xsl:element name="section">
              <xsl:attribute name="id"><xsl:value-of select="$localid"/></xsl:attribute>
              <xsl:attribute name="style">width:60%;padding;30px;</xsl:attribute>
              <!-- Determine dimensions -->               
              <xsl:element name="div">
                <xsl:attribute name="class">parent</xsl:attribute>
                <xsl:apply-templates select="@height|@width"/>
                <xsl:element name="div">
                  <xsl:attribute name="class">panzoom</xsl:attribute>
              ...
              <div class="buttons" style="text-align:center;padding-top:10px">
              ...
              <xsl:element name="script">
                (function() {
                var $section = $('#<xsl:value-of select="$localid"/>');
...
<xsl:template match="*[contains(@class, ' topic/image ')]/@height">
  ...
  <xsl:if test="not($height-in-pixel = '100%')">
    <xsl:if test="$TRANSTYPE = 'xhtml' or $TRANSTYPE = 'eclipsehelp' or $TRANSTYPE = 'webhelp'">    
    <xsl:attribute name="height">
      <!--xsl:choose>
...
<html>
    ...
    <head><xsl:value-of select="$newline"/>
      ...
      <xsl:call-template name="processHDF"/>        <!-- Add user HDF file, if specified -->
        ...
        <link rel="stylesheet" type="text/css" >
          <xsl:attribute name="href"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery-ui-1.12.1.custom/jquery-ui.css')"/></xsl:attribute>
        </link> 
        <xsl:text>
          
        </xsl:text> 
        <link rel="stylesheet" type="text/css" >
          <xsl:attribute name="href"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.10.16/css/dataTables.jqueryui.min.css')"/></xsl:attribute>
        </link>           
        <xsl:text>
          
        </xsl:text> 
        <link rel="stylesheet" type="text/css" >
          <xsl:attribute name="href"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.4.2/css/buttons.dataTables.css')"/></xsl:attribute>
        </link>
        <xsl:text>
          
        </xsl:text> 
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.12.4/jquery-1.12.4.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.10.16/js/jquery.dataTables.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.10.16/js/dataTables.jqueryui.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.1.4.2/js/dataTables.buttons.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jszip.3.1.3/jszip.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script> 
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/pdfmake.0.1.32/vfs_fonts.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>   
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/buttons.1.4.2/js/buttons.html5.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/buttons.1.4.2/js/buttons.print.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/buttons.1.4.2/js/buttons.colVis.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/jquery.panzoom-master/dist/jquery.panzoom.min.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
        ...
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/customjs/custom.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>  
        <xsl:text>
  ...
  <xsl:if test="number($WATERMARKING) = 1">
      <meta name="classification" 
        content=" ENCRYPTED_REPLACEMENT_WATERMARKING_STRING "/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="chapterBody">
    ...
    <body>
      ...
      <span id="browsesequence"><xsl:text> </xsl:text></span>
      ...
      <xsl:call-template name="gen-user-footer"/>       <xsl:if test="$TRANSTYPE = 'javahelp'">
        <xsl:call-template name="Javahelp-footer"/>    
      </xsl:if>
     
      <!-- include user's XSL running footer here -->
      <xsl:call-template name="processFTR"/>      <!-- Include XHTML footer, if specified -->
      <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-endprop ')]" mode="out-of-line"/>
    <xsl:call-template name="create-script-table-footer"/>
      
      <xsl:if test="$TRANSTYPE = 'xhtml' or $TRANSTYPE = 'eclipsehelp' or $TRANSTYPE = 'webhelp'">       
        <script type="text/javascript" language="javascript">
          <xsl:attribute name="src"><xsl:value-of select="concat($PATH2PROJ, $CSSPATH, 'jquery/browsesequence.js')"/></xsl:attribute>
          <xsl:text> </xsl:text>
        </script>
      </xsl:if>     
