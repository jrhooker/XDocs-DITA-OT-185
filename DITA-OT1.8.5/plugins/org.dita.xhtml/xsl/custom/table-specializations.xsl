...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class, ' struct-d/msg-name-main ')]/text()">
        <xsl:text> - </xsl:text><span class="msg-name-main">
            <xsl:value-of select="."/>
        </span>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class, ' struct-d/double ')]">       
            <xsl:apply-templates mode="positions"/>        
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' struct-d/msb ')]" mode="#all">
        <span class="msb">
            <xsl:apply-templates/> <xsl:text> : </xsl:text>
        </span>       
    </xsl:template>

    <xsl:template match="*[contains(@class,' struct-d/lsb ')]" mode="#all">
        <span class="lsb">
            <xsl:apply-templates/>
        </span>
    </xsl:template>
