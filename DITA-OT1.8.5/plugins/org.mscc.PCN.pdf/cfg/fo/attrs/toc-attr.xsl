...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:attribute-set name="__toc__topic__content">
        ...
        <xsl:attribute name="font-size"><xsl:value-of select="$toc.font.size"/></xsl:attribute>
    ...
    <xsl:attribute-set name="__toc__lists__content" use-attribute-sets="__toc__topic__content">
        <xsl:attribute name="color"><xsl:value-of select="$title.font.color"/></xsl:attribute>        
        <xsl:attribute name="font-size">12pt</xsl:attribute>        
        <xsl:attribute name="padding-top">2mm</xsl:attribute>     
    </xsl:attribute-set>
    ...
    <!-- Changed the following template to indent at zero on the first level of the TOC and parameterized the depth of the indent-->
    <xsl:attribute-set name="__toc__indent">
        <xsl:attribute name="margin-left">
            <xsl:variable name="level" select="count(ancestor-or-self::*[contains(@class, ' topic/topic ')])"/>
            <xsl:value-of select="concat(string((number($level) * number($tocindentinMM)) - number($tocindentinMM)), 'mm')"/>
        </xsl:attribute> 
    ...
    <xsl:attribute-set name="__toc__indent_hanging">
        <xsl:attribute name="margin-left">
            <xsl:variable name="level" select="count(ancestor-or-self::*[contains(@class, ' topic/topic ')])"/>
            <xsl:value-of select="(number($level) * number($tocindentinMM)) - number($tocindentinMM)"/>
        </xsl:attribute>
    </xsl:attribute-set>  
    
    <xsl:attribute-set name="__lof_lot__indent">
