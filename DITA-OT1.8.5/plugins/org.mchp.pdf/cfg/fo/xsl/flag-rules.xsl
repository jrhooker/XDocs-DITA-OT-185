...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
 ...
 -->
    <!-- filterFile declared to avoid erroring out when absent -->
    <xsl:param name="filterFile"/>
    <xsl:param name="flagsParams"/>
    ...
    <xsl:template name="getExtProps">
        ...
        <xsl:choose>
            <xsl:when test="contains($domains, 'a(props')">
                <xsl:text>,</xsl:text>
                <xsl:value-of
                    select="normalize-space(concat('props',substring-before(substring-after($domains,'a(props'), ')')))"/>
                <xsl:call-template name="getExtProps">
                    <xsl:with-param name="domains"
                        select="substring-after(substring-after($domains,'a(props'), ')')"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise/>
        </xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="getGeneralValue">
                    ...
                    <xsl:with-param name="propsPath"
                        select="normalize-space(substring-before($propsPath, $propName))"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="getGeneralValue">
        ...
        <xsl:choose>
            <xsl:when test="contains(@*[name()=$propParentName],concat($propName,'('))">
                <xsl:value-of
                    select="substring-before(substring-after(@*[name()=$propParentName],concat($propName,'(')),')')"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="contains($propsPath,' ')">
                        <xsl:call-template name="getGeneralValue">
                            <xsl:with-param name="propName" select="$propName"/>
                            <xsl:with-param name="propsPath"
                                select="normalize-space(substring-before($propsPath, $propParentName))"
                            />
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise/>
    ...
    <xsl:template name="flagcheck"> </xsl:template>
    ...
    <xsl:template name="ext-flagcheck">
        ...
        <xsl:choose>
            <xsl:when test="contains($props,',')">
                ...
                <xsl:if test="not($propsValue='')"> </xsl:if>
    ...
    <xsl:template name="getrules-parent">
        ...
        <xsl:variable name="props">
            <xsl:if test="contains($domains, 'a(props')">
                <xsl:value-of
                    select="normalize-space(substring-before(substring-after($domains,'a(props'), ')'))"
                />
            </xsl:if>
        </xsl:variable>
   ...
   If found and if the filterFile name was passed in,
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="contains($flag-att-path,' ')">
                    <xsl:call-template name="ext-gen-prop">
                        <xsl:with-param name="flag-att-path"
                            select="normalize-space(substring-before($flag-att-path, $propName))"/>
                        <xsl:with-param name="flag-att-val" select="$flag-att-val"/>
                    </xsl:call-template>
                </xsl:if>
            </xsl:otherwise>
    ...
    <xsl:template name="gen-prop">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="$flag-att='rev' and $flagsParams/val/revprop[@val=$firstflag][@action='flag']">
            ...
            <xsl:when test="$flagsParams/val/prop[@att=$flag-att][@val=$firstflag][@action='flag']">
                <xsl:copy-of
                    select="$flagsParams/val/prop[@att=$flag-att][@val=$firstflag][@action='flag']"
                />
            </xsl:when>
            <xsl:otherwise/>
        ...
        <xsl:choose>
            <xsl:when test="string-length($moreflags)>0">
                ...
                <xsl:call-template name="gen-prop">
                    <xsl:with-param name="flag-att">
                        <xsl:value-of select="$flag-att"/>
                    </xsl:with-param>
                    <xsl:with-param name="flag-att-val">
                        <xsl:value-of select="$moreflags"/>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise/>
                ...
                <xsl:choose>
                    <xsl:when test="string-length($morerevs)>0">
                        ...
                        <xsl:call-template name="find-active-rev-flag">
                            <xsl:with-param name="allrevs">
                                <xsl:value-of select="$morerevs"/>
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
                        <!-- no more values - none found -->
    ...
    <xsl:template match="prop|revprop" mode="conflict-check">
        ...
        <xsl:choose>
            <xsl:when
                test="(@color and @color!='' and $color!='' and $color!=@color)or(@backcolor and @backcolor!='' and $backcolor!='' and $backcolor!=@backcolor)">
