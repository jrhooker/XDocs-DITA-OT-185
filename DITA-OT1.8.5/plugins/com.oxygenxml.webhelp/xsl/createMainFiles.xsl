...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*" mode="copy-xhtml">
    <xsl:element name="{local-name()}">
      <xsl:apply-templates select="@* | node()" mode="copy-xhtml"/>
    </xsl:element>
  </xsl:template>
    ...
    <xsl:result-document href="{$fileName}" 
        ...
        <html>
          ...
          <xsl:if test="string-length($WEBHELP_SKIN_CSS) > 0">
            <link rel="stylesheet" type="text/css" href="{$PATH2PROJ}{$WEBHELP_SKIN_CSS}">
              <xsl:comment/>
            </link>
          </xsl:if>
          <!-- user custom CSS -->
        ...
        <body onload="javascript:showMenu('content');" style="overflow: hidden;">
            ...
            <xsl:if test="not($withFrames)">
              <div style="width: 100%; vertical-align: middle; text-align: center; height: 100%;">
                <div style="position: absolute; top:45%; left:25%;width: 50%; "> You must enable
                    javascript in order to view this page or you can go <a href="{concat('index_frames', $OUTEXT)}"
                ...
                <div id="productTitle">
                    <a id="customLogo">
                      <xsl:if test="string-length($WEBHELP_LOGO_IMAGE) > 0">
                        <xsl:attribute name="style">background-image:url('<xsl:value-of select="$WEBHELP_LOGO_IMAGE"/>'); display:inline-block</xsl:attribute>
                      </xsl:if>
                  ...
                  <div class="framesLink">
                    ...
                    <xsl:variable name="printLinkText"> 	 
                      ...
                      <div id="printlink"> 	 
                      <a onclick="printFrame('frm')" title="{$printLinkText}"></a> 	 
                    </div>
                    <div>
                      <a href="{concat('index_frames', $OUTEXT)}" id="oldFrames" title="{$oldFramesLinkText}"></a>
                   </div>
                  </div>
                  </xsl:if>
                </div>
                  ...
                  <div id="indexBlock" style="display:none;">
                    <form action="javascript:void(0)" id="indexForm">
                      <fieldset>
                        ...
                        <input type="text" name="search" value="" id="id_search" placeholder="{$localizedMessage}"/>
                      </fieldset>
                    </form>
                    <div id="iList">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:result-document href="{concat('oxygen-webhelp/indexterms', $OUTEXT)}"
  ...
  <xsl:template name="create-js-scripts" exclude-result-prefixes="#all">
    <xsl:param name="withFrames"/>
      <script type="text/javascript" src="oxygen-webhelp/resources/js/jquery-1.8.2.min.js"><xsl:comment/></script>
      ...
      <script type="text/javascript" src="oxygen-webhelp/search/nwSearchFnt.js"><xsl:comment/></script>
      <xsl:variable name="LANG" select="lower-case(substring($DEFAULTLANG, 1, 2))"/>
      <xsl:if test="$LANG = 'en' or $LANG = 'fr' or $LANG = 'de'">
        <script type="text/javascript" src="oxygen-webhelp/search/stemmers/{$LANG}_stemmer.js"><xsl:comment/></script>
      </xsl:if>
      <xsl:if test="$withFrames">
        <xsl:call-template name="create-search-js-scripts"/>
      </xsl:if>

      <script type="text/javascript" charset="utf-8" src="oxygen-webhelp/resources/localization/strings.js"><xsl:comment/></script>
      <script type="text/javascript" charset="utf-8" src="oxygen-webhelp/resources/js/localization.js"><xsl:comment/></script>
      <script src="./oxygen-webhelp/resources/js/browserDetect.js" type="text/javascript"><xsl:comment/></script>

      <script type="text/javascript" charset="utf-8" src="oxygen-webhelp/resources/js/parseuri.js"><xsl:comment/></script>
    <xsl:if test="not($withFrames)">
        <script type="text/javascript" charset="utf-8" src="oxygen-webhelp/resources/js/jquery.ba-hashchange.min.js"><xsl:comment/></script>
        <script type="text/javascript" src="oxygen-webhelp/resources/js/splitter.js"><xsl:comment/></script>
    </xsl:if>

    <script type="text/javascript" src="oxygen-webhelp/resources/js/log.js"><xsl:comment/></script>
    <script type="text/javascript" src="oxygen-webhelp/resources/js/toc.js"><xsl:comment/></script>
  ...
  <xsl:template name="create-search-js-scripts" exclude-result-prefixes="#all">
      <script type="text/javascript" src="oxygen-webhelp/search/htmlFileList.js"><xsl:comment/></script>
      <script type="text/javascript" src="oxygen-webhelp/search/htmlFileInfoList.js"><xsl:comment/></script>
      <script type="text/javascript" src="oxygen-webhelp/search/index-1.js"><xsl:comment/></script>
      <script type="text/javascript" src="oxygen-webhelp/search/index-2.js"><xsl:comment/></script>
      <script type="text/javascript" src="oxygen-webhelp/search/index-3.js"><xsl:comment/></script>
  </xsl:template>
  ...
  <xsl:template match="b|strong|i|em|u|sub|sup">
    ...
    <xsl:variable name="firstTopic"
      select="($toc//toc:topic[@href and not(contains(@href, 'javascript'))])[1]/@href"/>
        <xsl:result-document href="{concat('index_frames', $OUTEXT)}" 
                                method="xhtml" 
                                indent="no" 
                                encoding="UTF-8"
                                doctype-system="about:legacy-compat"
      ...
      <html>
        ...
        <head>
          ...
          <script type="text/javascript">
              <xsl:comment>
                        function getContextId() {
                           ...
                           if (page != "") {
                               page = page.split("=");
                               if (page[0] == 'contextId') contextId = page[1];
                           }
                           contextId = "toc.html?contextId=" + contextId;
          ...
          <xsl:choose>
              <xsl:when test="starts-with($lang-attribute/lang/@xml:lang, 'ar') 
