...
class User
{
    ...
    function listUsers()
    {
        ...
        if ($this->ldap instanceof Ldap) {
            $ldapUsers = array();
            try {
        		$ldapUsers = $this->ldap->listAllUsers(array(LDAP_ACCOUNT_EMAIL, LDAP_ACCOUNT_FULLNAME), 0);
            } catch (Exception $e) {
        ...
        foreach ($ldapUsers as $key=>$user) {
            $i++;
            if ($key != 'count') {
                $uid = str_replace(',' . LDAP_ACCOUNT_BASE, "", $user['dn']);
                    $uInfo = explode('=', $uid, 2);
                    $un = $uInfo[1];
