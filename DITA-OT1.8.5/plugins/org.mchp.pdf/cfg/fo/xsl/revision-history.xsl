        ...
        <fo:block>     
            <xsl:for-each select="$revisions">
                <xsl:sort select="*[contains(@class, ' pmcrevhis-d/pmc-rev-number ')]"
                    order="descending"/>
                <fo:block keep-with-next="always" xsl:use-attribute-sets="revision.title">Revision <xsl:value-of
                    select="*[contains(@class, ' pmcrevhis-d/pmc-rev-number ')]"/></fo:block>
                <fo:block keep-with-next="always" xsl:use-attribute-sets="p">Revision <xsl:value-of
